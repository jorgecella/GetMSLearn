...
webURL = 'https://docs.microsoft.com/es-mx/learn/modules/intro-to-azure-networking/'
...
for cssfiles in css_files:
    ...
    if 'css' in cssOK:
        if not os.path.exists(path + '_themes'):
            os.mkdir(path + '_themes')
        if not os.path.exists(path + '_themes\\' + cssfiles.split('/')[-1]):
            download('https://docs.microsoft.com' + cssfiles[0:cssfiles.rfind('/')] + '/' + cssfiles.split('/')[-1].split('.')[-2] + '.css'  , path + '_themes\\' + cssfiles.split('/')[-1].split('.')[-2] + '.css'  )
        # change links to css
...
for linksImages in image_files:
        ...
        if not os.path.exists(linksImages):
                ...
                with open(path + '\media\\' + linksImages.split('/')[-1].split('.')[-2] + '.' + linksImages.split('/')[-1].split('.')[-1]  , 'wb') as f:
                        f.write(r.content)
                filename = filename.replace(linksImages, 'media/' + linksImages.split('/')[-1].split('.')[-2] + '.' + linksImages.split('/')[-1].split('.')[-1] )

# download webpage
with io.open(path + "index.html", "w") as f:
    f.write(filename)
